...
/*
  - This event struct is etcd-specific.
    Therefore, for every new system, a new event type is necessary
  - It doesn't need to be event type per TLA+ spec,
    since N TLA+ spec for N analysis purposes could re-use the same system event.
    But if two TLA+ specs are for different parts of the system and
    they have different event, we may have EventA, EventB and so on.
*/
type Event struct {
	Name   string
	Node   uint64 `json:"-"`
